...
sql =			" Select "
sql =	sql &	" REGNO,TITLE,P_TOP,P_LEFT,POSITION,WIDTH,HEIGHT,ZINDEX,ISUSE,ISDRAG,ISUSEDATE,ST_DATE,ED_DATE,WORK_USER,WORK_NAME,WORK_DATE,POPUP_STATUS,"
sql =	sql &	" dbms_lob.substr(content,8000,1) as content"
sql =	sql &	" from web_popup_L where regno = " & sid
'Response.WRite sql
...
sP_TOP= rs("P_TOP")
sP_LEFT= rs("P_LEFT")
sPOSITION= rs("POSITION")
sWIDTH= rs("WIDTH")
sHEIGHT= rs("HEIGHT")
sZINDEX= rs("ZINDEX")
sISUSE= rs("ISUSE")
sISDRAG= rs("ISDRAG")
sISUSEDATE= rs("ISUSEDATE")
sST_DATE= rs("ST_DATE")
sED_DATE= rs("ED_DATE")
sWORK_USER= rs("WORK_USER")
sWORK_NAME= rs("WORK_NAME")
sWORK_DATE= rs("WORK_DATE")
sPOPUP_STATUS= rs("POPUP_STATUS")
contents= rs("CONTENT")
...
'	sql = sql & " where bbs_kind = '" & bbs_kind & "'	"
	...
	If DBConn.Errors.Count = 0 Then
		DBConn.CommitTrans
		Response.Write "<script language=javascript>"
		Response.Write "	alert('게시물이 삭제 되었습니다.');"
		Response.Write "	location.href = './admin_popup.asp?" & link_str & "';"
		Response.Write "</script>"
		Response.End
	Else
...
<head>
	...
	$(document).ready(function() {
		...
		$("#st_date, #ed_date").datepicker({
			...
			buttonImage: "/common/images/find_btn_cal.gif",
...
<table class="tbl_write_main">
	...
	<input type="hidden" name="update_name"	value="<%=session("user_name")%>">
	...
	<%
	'in bbs_common.inc
	Response.Write typeWriteTable(bbs_type)
	%>
	...
	<tr class="write_tr">
		...
		<td class="write_td_content">
					...
					name="pop_left" id="pop_left"  value="<%=sP_LEFT%>"
					check="T" check_type="length" check_length="0" check_key=""
					title="좌측 외치를 설정해주세요.">
		</td>
		...
		<td class="write_td_content">
					<input	type="text" class=""
					name="pop_top" id="pop_top" value="<%=sP_TOP%>"
					check="T" check_type="length" check_length="0" check_key=""
					title="상단 외치를 설정해주세요.">
		</td>
	</tr>
	...
	<tr class="write_tr">
		...
		<td class="write_td_content">
					<input	type="text" class=""
					name="pop_width" id="pop_width" value="<%=sWIDTH%>"
					check="T" check_type="length" check_length="0" check_key=""
		...
		<td class="write_td_content">
					<input	type="text" class=""
					name="pop_height" id="pop_height" value="<%=sHEIGHT%>"
					check="T" check_type="length" check_length="0" check_key=""
	...
	<tr class="write_tr">
		...
		<td class="write_td_content">
			<input	type="text" class=""
					name="pop_zindex" id="pop_zindex" value="<%=sZINDEX%>"
					check="T" check_type="length" check_length="0" check_key=""
		...
		<td class="write_td_content">
				<select name="popup_drag" id="popup_drag"
					class="select">
				<option value="0" <%If sISDRAG = "0" Then Response.Write "selected" End if %>>고정</option>
				<option value="1" <%If sISDRAG = "1" Then Response.Write "selected" End if %>>이동</option>
			</select>
	...
	<tr class="write_tr">
		...
		<td class="write_td_content">
			<select name="pop_usedate" id="pop_usedate"
					class="select">
				<option value="0" <%If sISUSEDATE = "0" Then Response.Write "selected" End if %>>일반</option>
				<option value="1" <%If sISUSEDATE = "1" Then Response.Write "selected" End if %>>날짜</option>
			</select>
		</td>
		...
		<td class="write_td_content">
				<select name="popup_status" id="popup_status"
					class="select">
				<option value="0" <%If sPOPUP_STATUS = "0" Then Response.Write "selected" End if %>>보류</option>
				<option value="1" <%If sPOPUP_STATUS = "1" Then Response.Write "selected" End if %>>게시</option>
				<option value="2" <%If sPOPUP_STATUS = "2" Then Response.Write "selected" End if %>>종료</option>
			</select>
	...
	<tr class="write_tr">
		...
		<td class="write_td_colspan" colspan="3">
			<input	type="text"
					name="subject" id="subjecct" value="<%=sTITLE%>"
					check="T" check_type="length" check_length="0" check_key=""
					title="글제목을 입력해 주십시오."
	...
	<tr class="submit_tr">
		<td colspan="4" class="submit_td">
			...
			<img src="./img/btn_del.gif"  border="0" style="cursor:pointer;" onClick="delPopup();">
			</div>
		</td>
	</tr>
	</form>
...
function frm_chk() {
	...
	if (proc_ok == "T")	{
		...
		frm.target	="proc_fr";
		frm.action	= "./admin_popup_edit_ok.asp";
		frm.submit();
	}
}
...
function delPopup() {
	if(confirm('팝업을 삭제하시겠습니까?')){
		var url_	= "./admin_popup_edit.asp?mode=DEL&sid="+<%=sid%>;
		location.href = url_;
	}

...
<table id="tbl_search">
	...
	<tr>
		<td class="search_td" style="width:100%;padding:0px;">
			<b style="vertical-align:middle;">구분:</b>
			<select name="srch_cate" style="height:20px;vertical-align:middle;margin-right:10px;">
				<option value="">전체</option>
				<%
				sql =		"select	DISTINCT cate_kind	"
				sql = sql & "  from	web_faq_cate		"
				sql = sql & " order by cate_kind		"
				Set rs = DBConn.Execute(sql)
				Do While Not rs.EOF
					If srch_cate = rs("cate_kind") Then
						strSelected = " selected"
					Else
						strSelected = ""
					End If
					Response.Write "<option value=""" & rs("cate_kind") & """" & strSelected & ">" & rs("cate_kind") & "</option>"
					rs.MoveNext
				Loop
				%>
			</select>
			<select name="srch_key" style="height:20px;vertical-align:middle;">
				<option value="subject" <% If srch_key = "cate_cd"	Then Response.Write "selected" %>>코드</option>
				<option value="content" <% If srch_key = "cate_name"	Then Response.Write "selected" %>>카테고리명</option>
	        </select>
	...
	<tr class="header_tr">
		<td class="header_no"		>코드</td>
		<td class="header_no"		>구분</td>
		<td class="header_writer"	>카테고리</td>
		<td class="header_writer"	>사용</td>
		<td class="header_writer"	>등록일</td>
		<td class="header_writer"	>등록자</td>
		<td class="header_date"		>노출순서</td>
	</tr>
	...
	%>
			<tr>
				<td colspan="7" class="no_list">등록된 게시물이 없습니다.</td>
			</tr>
	<%
	Else
		'----------------------------------------------------------------------- Main Article S
		...
		Do While (Not rs.eof) And (Rcount > 0 )
			...
			If isuse = "1" Then
				htmlIsuse = "<label style=""color:blue;"">" & isuses & "</label>"
			Else
				htmlIsuse = "<b style=""color:red;"">" & isuses & "</b>"
			End If


			cate_name		= "<span style=""cursor:pointer;"" onClick=""view_article('" & cate_cd & "','" & isSecret & "');"">" & repl_badge & cate_name & new_badge & "</span>"
		%>
			<tr class="list_tr" onMouseOver="this.className='list_tr_over';" onMouseOut="this.className='list_tr';">
				<td class="list_no"		/><%=cate_cd	%></td>
				<td class="list_no"		/><%=cate_kind	%></td>
				<td class="list_writer"	/><%=cate_name	%></td>
				<td class="list_no"		/><%=htmlIsuse	%></td>
				<td class="list_writer"	/><%=work_date	%></td>
				<td class="list_writer"	/><%=work_user	%></td>
				<td class="list_date"	/><%=order_view	%></td>
			</tr>
		<%
			cnt		= cnt + 1
			tempNo	= tempNo - 1
	...
	End If
		'----------------------------------------------------------------------- Action Button S
	%>
	<tr class="blank_tr"><td colspan="7"></td></tr>
	<tr>
		<td class="list_btn" colspan="7">
			<%

			If session("admin") <> "" Then
				Response.Write "<img src=""../img/btn_save.gif"" style=""cursor:pointer;"" onClick=""location.href='./faq_cate_write.asp?" & Link_Str & "';"">"
	...
	%>
	<tr class="blank_tr"><td colspan="8"></td></tr>
...
function view_article(a_id, a_lock) {
	...
	var url_	= "./faq_cate_modify.asp?" + query;
